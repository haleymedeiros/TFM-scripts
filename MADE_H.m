...
clc % clear matlab command windowç
...
addpath(genpath('C:\Users\haleymedeiros\Applications\eeglab2019_1'));
rawdata_location = '/Users/haleymedeiros/Desktop/wed';

channel_locations = '/Users/haleymedeiros/Desktop/eeglab2019_1/sample_locs/GSN129.sfp';
output_location= '/Users/haleymedeiros/Desktop/EEG/practice'; %% use me

%%if you want to load data manually
%EEG = pop_mffimport({'/Users/haleymedeiros/Desktop/wed/adult2.mff'},{'classid' 'code' 'description' 'label' 'mffkey_cidx' 'mffkey_gidx' 'mffkeys' 'mffkeysbackup' 'name' 'relativebegintime' 'sourcedevice' 'tracktype'});
...
%sampling_rate = ; % set sampling rate (in Hz), if you want to down sample
...
% 7. Initialize the filters
 % High-pass frequency
 % Low-pass frequency. We recommend low-pass filter at/below line noise frequency (see manuscript for detail)
highpass = 0.3;
lowpass = 50;
...
% catalan 
task_event_markers = {EEG.event.code}; need to put this in the loop, as
%Ranjan told us.
...
task_epoch_length = [0 3.82]; % epoch length in second
...
volt_threshold = [-1000, 1000]; % lower and upper threshold (in ?V)
...
frontal_channels = {'E1' 'E8' 'E14' 'E21' 'E25' 'E32'}; % If you set interp_epoch = 1, enter the list of frontal channels to check (see manuscript for detail)
...
%%[filepath,name,ext] = fileparts(char(datafile_names));
...
for subject=1:length(datafile_names)
    ...
    task_event_markers = {EEG.event.type}; % changed this to code on 6/9/2020
...
%     EEG = pop_select( EEG,'nochannel', 65:72); % delete redundant channels
    ...
    % pop_firws() - filter window type hamming ('wtype', 'hamming')
    % pop_firws() - applying zero-phase (non-causal) filter ('minphase', 0)t( EEG );
    ...
    if all_bad_channels == 1
        ...
        total_epochs_after_artifact_rejection(subject)=0;
        total_channels_interpolated(subject)=0;
        %continue % ignore rest of the processing and go to next
        %datafile-ONLY FOR MULTIPLE SUBJECTS*
    end
...
filename = 'adult_1.xlsx'; writetable(report_table,filename,'Sheet',1)

%% Filtering before Hilbert transform
...
%EEG=butterfilt(EEG,[0.3 8],[-100 0],0);
...
%filteredData = filtfilt(b, a, ALLEEG); % Apply filter to data using zero-phase filtering
